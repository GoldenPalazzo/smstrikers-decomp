...
void Goalie::DoNavigation(float fDeltaT, float fIdleDistance, Goalie::eNaviMode naviMode)
{
    ...
    desiredAng = (u16)(s32)(10430.378f * nlATan2f(mv3LocalNavTarget.f.y, mv3LocalNavTarget.f.x));
...
void Goalie::FindDesiredGoaliePosition(nlVector3& pos, nlVector3& dir, nlVector3& focus, unsigned short& ang, const nlVector3* pThreatPos)
{
    ...
    desiredPos.f.x = desiredPos.f.x >= -goalLine ? desiredPos.f.x : -goalLine;
    desiredPos.f.x = desiredPos.f.x <= goalLine ? desiredPos.f.x : goalLine;

    const nlVector3& rPos = m_v3Position;
    pos.f.x = 0.8f * desiredPos.f.x + 0.2f * rPos.f.x;
    pos.f.y = 0.8f * desiredPos.f.y + 0.2f * rPos.f.y;
    pos.f.z = 0.8f * desiredPos.f.z + 0.2f * rPos.f.z;
    dir = desiredVec;
    focus = targetPos;
}
...
inline bool Goalie::IsLooseBallTowardNet()
{
    nlVector3 v3BallVel;
    nlVector3 v3Post1;
    nlVector3 v3Post2;
...
inline void Goalie::CleanupStun()
{
...
inline void Goalie::CleanSTSRecover()
{
    mpSaveData = NULL;
    CleanupStun();
}

inline void Goalie::InitActionSnapBall()
{
    CleanGoalieAction();
    mPrevGoalieActionState = mGoalieActionState;
...
inline void Goalie::InitActionSTSAttackSetup(f32 fWaitTime)
{
    mfWaitTime = fWaitTime;
    mfTargetTime = fWaitTime >= 0.25f ? fWaitTime : 0.25f;
...
inline void Goalie::InitActionOffplay(eGoalieOffplayType type)
{
    CleanGoalieAction();
    mPrevGoalieActionState = mGoalieActionState;
...
inline void Goalie::MakeExertEvent()
{
...
inline bool Goalie::CheckForBallOnHead()
{
    if (g_pBall->m_tNoPickupTimer.m_uPackedTime == 0
...
inline u16 Goalie::FindDumpDirection(u16 aDesired, bool bConstrain)
{
    if (bConstrain)
    {
...
inline void Goalie::InitActionLooseBallPursueBouncing(const nlVector3& v3TargetPosition, float fTargetTime)
{
    CleanGoalieAction();
    mPrevGoalieActionState = mGoalieActionState;
...
int Goalie::ChooseRunAnim(short nAngle, const nlVector3& rTargetPos, float fThreshold)
{
    ...
    if (nlGetLengthSquared2D(v3Delta.f.x, v3Delta.f.y) < (fThreshold * fThreshold))
    {
        mMoveDirection = GOALIEDIR_IDLE;
        return 8;
    }
...
inline void Goalie::HandleSTSSwat()
{
