...
FuzzyVariant Fuzzy::GoodPassTargetFrom(cFielder* TheTargetFielder, cFielder* TheBallOwner)
{
    ...
    if (fFalseConfidence > 0.0f)
    {
        ...
        float fBranchRatioNear = (fTrueConfidenceNear <= fFalseConfidenceNear) ? fTrueConfidenceNear : fFalseConfidenceNear;
        fBranchRatioNear = fBranchRatioNear / ((fTrueConfidenceNear >= fFalseConfidenceNear) ? fTrueConfidenceNear : fFalseConfidenceNear);
...
FuzzyVariant Fuzzy::GetBestPassReceiveAction(cFielder* TheFielder)
{
    ...
    if (fTrueConfidence > 0.0f)
    {
        ...
        fDanger = (fDanger >= fTrueConfidence) ? fDanger : fTrueConfidence;
        fDanger = (fClosingTo >= fDanger) ? fClosingTo : fDanger;

        float fNearTo = NearTo((cPlayer*)TheFielder, (cPlayer*)pClosestOpponent);
        float fOnScreen = OnScreen((cPlayer*)pClosestOpponent);

        fNearTo = (fNearTo <= fDanger) ? fNearTo : fDanger;
        fOnScreen = (fOnScreen <= fNearTo) ? fOnScreen : fNearTo;
        ...
        if (fTrueConfidence > 0.0f)
        {
            ...
            if (fTrueConfidence > 0.0f)
            {
                ...
                fNearFar = (fNearFar <= fClosing3) ? fNearFar : fClosing3;

                float fDangerDelayed = FGREATER(InDangerDelayed(TheFielder).mData.f, 0.0f);
                float fNotWide = 1.0f - WideOpen(TheFielder);

                fDangerDelayed = (fDangerDelayed >= fNearFar) ? fDangerDelayed : fNearFar;
                fNotWide = (fNotWide >= fDangerDelayed) ? fNotWide : fDangerDelayed;
                ...
                fOneTimerLess = (fOneTimerLess <= fNotWide) ? fOneTimerLess : fNotWide;
                fNearTarget = (fNearTarget <= fOneTimerLess) ? fNearTarget : fOneTimerLess;
                fPassConf = (fPassConf <= fNearTarget) ? fPassConf : fNearTarget;
            ...
            if (fVolleyFalse > 0.0f)
            {
                ...
                fNearFar2 = (fNearFar2 <= fClosing4) ? fNearFar2 : fClosing4;

                float fDangerDelayed2 = InDangerDelayed(TheFielder).mData.f;
                fDangerDelayed2 = (fDangerDelayed2 >= fNearFar2) ? fDangerDelayed2 : fNearFar2;
                ...
                fNearTarget2 = (fNearTarget2 <= fDangerDelayed2) ? fNearTarget2 : fDangerDelayed2;
                fPassConf2 = (fPassConf2 <= fNearTarget2) ? fPassConf2 : fNearTarget2;
 ...
 * TODO: 96.97% match - remaining diffs are register/branch ordering in confidence gates.
 */
FuzzyVariant Fuzzy::GetBestLooseBallAction(cFielder* TheFielder)
{
    ...
    if (fTrueConfidence > 0.0f)
    {
        ...
        float fGreater2 = FGREATER(bestPassTargetFielder.Confidence, 0.3f);
...
FuzzyVariant Fuzzy::GetBestWindupShotAction(cFielder* TheFielder)
{
    ...
    float fGoalieStunned = Stunned(pGoalie);
    fTrueConfidence = (fGoalieStunned >= fTrueConfidence) ? fGoalieStunned : fTrueConfidence;
    ...
    if (fTrueConfidence > 0.0f)
    {
        ...
        fAttacked = (fAttacked >= fNeedDeke) ? fAttacked : fNeedDeke;
        fNotCloseToTheirGoalie = (fNotCloseToTheirGoalie <= fNotCloseToSideline) ? fNotCloseToTheirGoalie : fNotCloseToSideline;
        fNotRepeatingLastDesire = (fNotRepeatingLastDesire <= fNotCloseToTheirGoalie) ? fNotRepeatingLastDesire : fNotCloseToTheirGoalie;
        fAttacked = (fAttacked <= fNotRepeatingLastDesire) ? fAttacked : fNotRepeatingLastDesire;
    ...
    fTrueConfidence = (fNearToTheirGoalie >= fTrueConfidence) ? fNearToTheirGoalie : fTrueConfidence;
