...
void cPlayer::DoRegularPassing(cPlayer* pTeammate, bool bVolleyPass, bool bAllowLeadPass, bool bParam3, bool bParam4)
{
    ...
    else
    {
        ...
        velocity.f.z = fInvSpeed * (v3PassIntercept.f.z - g_pBall->m_v3Position.f.z);
        velocity.f.y = fInvSpeed * (v3PassIntercept.f.y - g_pBall->m_v3Position.f.y);
        velocity.f.x = fInvSpeed * (v3PassIntercept.f.x - g_pBall->m_v3Position.f.x);
