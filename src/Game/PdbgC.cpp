...
void cPlayer::DoRegularPassing(cPlayer* pTeammate, bool bVolleyPass, bool bAllowLeadPass, bool bParam3, bool bParam4)
{
    ...
    if (bAllowLeadPass)
    {
        if (pPassTarget->ShouldILeadPass())
        {
            ...
            float fSearchScore = pPassTarget->m_pSpaceSearch->FindBestPosition(suggestedPassTarget, pPassTarget->m_v3Position, (eFieldDirection)direction, &m_v3Position, 6.0f, 0xAAAA);
            ...
            if (distSq2D > 1.0f)
            {
                ...
                {
                    nlPolarToCartesian(teammateLeadPassVelocity.f.x, teammateLeadPassVelocity.f.y, pPassTarget->m_aActualFacingDirection, fRunningSpeed);
                    bLeadPass = true;
                    teammateLeadPassVelocity.f.z = 0.0f;
                }
                else
                {
                    float fLength = nlSqrt(pSuggestedPassDirection->f.x * pSuggestedPassDirection->f.x + pSuggestedPassDirection->f.y * pSuggestedPassDirection->f.y + pSuggestedPassDirection->f.z * pSuggestedPassDirection->f.z, true);
                ...
                if (bVolleyPass || bParam4)
                {
                    ...
                    float fMinPassSpeed = m_pTweaks->fPassGroundSpeedMin, fMaxPassSpeed = m_pTweaks->fPassGroundSpeedMax, adjustedSpeed = DoCalculatePassSpeed(teammatePosition, teammateLeadPassVelocity, m_pTweaks->fPassVolleySpeedMin, m_pTweaks->fPassVolleySpeedMax, g_pGame->m_pGameTweaks->fPassSpeedMinDist, g_pGame->m_pGameTweaks->fPassSpeedMaxDist);
                ...
                {
                    const nlVector3& teammatePosition = pPassTarget->m_v3Position;
                    float fMinPassSpeed = m_pTweaks->fPassGroundSpeedMin, fMaxPassSpeed = m_pTweaks->fPassGroundSpeedMax, adjustedSpeed = DoCalculatePassSpeed(teammatePosition, teammateLeadPassVelocity, m_pTweaks->fPassGroundSpeedMin, m_pTweaks->fPassGroundSpeedMax, g_pGame->m_pGameTweaks->fPassSpeedMinDist, g_pGame->m_pGameTweaks->fPassSpeedMaxDist);
                    adjustedSpeed = (adjustedSpeed >= fMinPassSpeed) ? adjustedSpeed : fMinPassSpeed;
                    fPassGroundSpeed = (adjustedSpeed <= fMaxPassSpeed) ? adjustedSpeed : fMaxPassSpeed;
                }
                if (!bLeadPass)
                ...
                if (bLeadPass)
                {
                    ...
                    CalcInterceptXY(g_pBall->m_v3Position, fPassGroundSpeed, 0.0f, pPassTarget->m_v3Position, teammateLeadPassVelocity, interceptResult, interceptTimes);
    ...
    if (!calcPassIntercept)
    {
        ...
        {
            const nlVector3& teammatePosition = pPassTarget->m_v3Position;
            float fMinPassSpeed = m_pTweaks->fPassGroundSpeedMin, fMaxPassSpeed = m_pTweaks->fPassGroundSpeedMax, adjustedSpeed = DoCalculatePassSpeed(teammatePosition, pPassTarget->m_v3Velocity, m_pTweaks->fPassVolleySpeedMin, m_pTweaks->fPassVolleySpeedMax, g_pGame->m_pGameTweaks->fPassSpeedMinDist, g_pGame->m_pGameTweaks->fPassSpeedMaxDist);
            adjustedSpeed = (adjustedSpeed >= fMinPassSpeed) ? adjustedSpeed : fMinPassSpeed;
            fPassGroundSpeed = (adjustedSpeed <= fMaxPassSpeed) ? adjustedSpeed : fMaxPassSpeed;
        }
        else
        {
            const nlVector3& teammatePosition = pPassTarget->m_v3Position;
            float fMinPassSpeed = m_pTweaks->fPassGroundSpeedMin, fMaxPassSpeed = m_pTweaks->fPassGroundSpeedMax, adjustedSpeed = DoCalculatePassSpeed(teammatePosition, pPassTarget->m_v3Velocity, m_pTweaks->fPassGroundSpeedMin, m_pTweaks->fPassGroundSpeedMax, g_pGame->m_pGameTweaks->fPassSpeedMinDist, g_pGame->m_pGameTweaks->fPassSpeedMaxDist);
            adjustedSpeed = (adjustedSpeed >= fMinPassSpeed) ? adjustedSpeed : fMinPassSpeed;
            fPassGroundSpeed = (adjustedSpeed <= fMaxPassSpeed) ? adjustedSpeed : fMaxPassSpeed;
        }
    }
